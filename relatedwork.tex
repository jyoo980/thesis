...
question that is often asked after a refactor.
...
A fundamental question that developers often ask during software evolution
tasks is: ``what is relevant?" \cite{sillito-2008-asking-and-answering}.
Tools that provide support for developer tasks such as structural search tools
or call graphs often present an incredible amount of information that may not
necessarily be relevant to a task.
This introduces friction between the developer and their tooling, which in
turn may negatively impact the developer's perceived utility of the tools
that are available to them, no matter how useful they may actually be.

% Say a little bit about how tools exist already that help answer reach q.
% The problem isn't the lack of tooling, the problem is the lack of having
% useful information about the task at hand.

\subsection{Low-level Support for Reachability Questions}
\label{subsec:LowLevelSupportReachabilityQuestions}

TODO: talk about slicing/traces/how IDEs already support reachability q, but
aren't super helpful (in terms of how they visualize data).
